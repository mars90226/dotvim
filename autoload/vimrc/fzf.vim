...
function! vimrc#fzf#shell_outputs_in_commandline(...) abort
  ...
  " NOTE: temporarily disable completion while fzf injects cmdline
  lua require("vimrc.plugins.blink_cmp").disable()
...
function! vimrc#fzf#helptags(...) abort
  ...
  call writefile(['/('.(v:lua.require("vimrc.check").os_is('windows') ? '^[A-Z]:\/.*?[^:]' : '.*?').'):(.*?)\t(.*?)\t/; printf(qq('.vimrc#fzf#green('%-40s', 'Label').'\t%s\t%s\n), $2, $3, $1)'], s:helptags_script)
